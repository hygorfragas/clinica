...
description: >-
  Guides craft-oriented interface design for dashboards, admin panels, SaaS apps,
  tools, settings and data UIs (not marketing landings). Loads and maintains
  `.interface-design/system.md` for cross-session consistency. Use when the user
  builds or refines product UI, design tokens, spacing/depth systems, or asks for
  design critique, audit against a system, or extraction of UI patterns.
---
...
For more detail on specific topics (paths relative to this skill folder):
...
- `references/example.md` — Additional examples

# Cursor: fluxos (equivalente aos comandos do Claude Code)

No Cursor não há slash-commands do plugin. Quando o usuário pedir uma destas ações (ou sinônimos em português), **ler e seguir** o arquivo em `workflows/`:

| Pedido típico | Arquivo |
| --- | --- |
| Estado do design system / status | `workflows/status.md` |
| Auditar código contra o system | `workflows/audit.md` |
| Extrair padrões do código → `system.md` | `workflows/extract.md` |
| Crítica de craft / refinar o que foi feito | `workflows/critique.md` |
| Começar UI alinhado ao fluxo completo | `workflows/init.md` |

**Instalação em outro projeto:** copie a pasta inteira `.cursor/skills/interface-design/` (esta skill) para `<projeto>/.cursor/skills/interface-design/` — cópia real de arquivos, não link simbólico. Opcional: copie também `.cursor/rules/interface-design.mdc` para `<projeto>/.cursor/rules/`.
