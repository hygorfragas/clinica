...
export default async function PacienteAnamnesePage({ params }: PageProps) {
  ...
  const [{ data: templatesRows }, { data: submissionRows }] = await Promise.all([
    ctx.supabase
      ...
      .select("id, name, description, pdf_storage_path, page_count, form_schema, is_default")
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-xl font-semibold text-ink">Anamnese & consentimentos</h2>
          <p className="text-sm text-ink-muted">
            Escolha um template para preencher ou continue um rascunho.
          </p>
        </div>
        {templates.length === 0 && (
          <Link
