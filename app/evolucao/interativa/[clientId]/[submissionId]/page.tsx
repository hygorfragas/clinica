...
export default async function EvolucaoInterativaPage({ params }: PageProps) {
  ...
  if (submission.status !== "draft") {
    redirect(`/pacientes/${clientId}/evolucao`);
  }
  ...
  if (submission.template_id) {
    ...
    if (template?.pdf_storage_path) {
      ...
      if (signErr) {
        console.error(
          "[evolucao.interativa] Falha ao gerar signed URL:",
          signErr.message,
        );
      }
