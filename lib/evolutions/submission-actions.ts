...
import {
  buildEvolutionSubmissionStoragePath,
  CLINICAL_BUCKET,
...
export type SaveEvolutionSubmissionInput = z.infer<typeof saveInputSchema>;

export async function saveEvolutionSubmission(
  clientId: string,
  input: SaveEvolutionSubmissionInput,
  ...
  if (!parsed.success) {
    return {
      ok: false,
      error:
        parsed.error.issues[0]?.message ?? "Dados de evolução inválidos.",
    };
  }
  ...
  if (parsed.data.submissionId) {
    ...
    if (error || !data) {
      return {
        ok: false,
        error: error?.message ?? "Falha ao salvar evolução.",
      };
    }
...
export async function submitEvolution(
  ...
  if (!submission) return { ok: false, error: "Evolução não encontrada." };
  ...
  const storagePath = buildEvolutionSubmissionStoragePath({
...
async function flattenSubmission(params: {
  ...
  for (const stroke of inkStrokes) {
    ...
    const cfg = getStrokeRenderConfig(
      stroke.tool,
      stroke.width ?? 1.6,
      stroke.opacity,
    );
  ...
  if (signerName) {
    ...
    last.drawText(
      `Registrado por: ${signerName} — ${new Date().toLocaleString("pt-BR")}`,
      {
        x: 40,
...
export async function deleteEvolutionSubmission(
  clientId: string,
  submissionId: string,
): Promise<Ok | Err> {
  const ctx = await requireClinicalTenantContext();
  ...
  const { data: sub } = await ctx.supabase
    ...
    .eq("tenant_id", ctx.tenantId)
    .maybeSingle();
  if (!sub) return { ok: false, error: "Evolução não encontrada." };
