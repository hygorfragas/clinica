...
import {
  buildAnamnesisTemplateStoragePath,
  CLINICAL_BUCKET,
  MAX_DOCUMENT_BYTES,
...
export async function createAnamnesisTemplate(
  ...
  if (!isFormDataFileLike(file) || file.size === 0) {
    return { ok: false, error: "Selecione o arquivo PDF do template." };
  }
  if (file.size > MAX_DOCUMENT_BYTES) {
    return { ok: false, error: "Arquivo muito grande (máx. 20 MB)." };
  }
  if (file.type !== "application/pdf") {
    return { ok: false, error: "O template deve ser um arquivo PDF." };
  }

  const isDefault =
    isDefaultRaw === "1" || isDefaultRaw === "true" || isDefaultRaw === "on";
  const pageCount = Number(pageCountRaw);
  const pageCountSafe = Number.isFinite(pageCount) && pageCount > 0
    ? Math.max(1, Math.floor(pageCount))
    : 1;

  const templateId = randomUUID();
  const storagePath = buildAnamnesisTemplateStoragePath({
  ...
  if (error || !data) {
    ...
    return {
      ok: false,
      error: error?.message ?? "Não foi possível criar o template.",
    };
  }
...
export async function updateAnamnesisTemplate(
  ...
  if (input.inkRegions !== undefined) {
    ...
    if (!parsed.success) {
      return {
        ok: false,
        error: "Áreas de desenho inválidas.",
      };
    }
  ...
  if (error) {
    return {
      ok: false,
      error: error.message ?? "Não foi possível atualizar o template.",
    };
  }
...
export async function deleteAnamnesisTemplate(
  ...
  if (error) {
    return {
      ok: false,
      error: error.message ?? "Não foi possível excluir o template.",
    };
  }
...
export async function getTemplateSignedUrl(
  ...
  const { data: signed, error } = await ctx.supabase.storage
    .from(CLINICAL_BUCKET)
    .createSignedUrl(tpl.pdf_storage_path, 60 * 30);
