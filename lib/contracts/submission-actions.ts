...
import {
  buildContractSubmissionStoragePath,
  CLINICAL_BUCKET,
...
export type SaveContractSubmissionInput = z.infer<typeof saveInputSchema>;

export async function saveContractSubmission(
  clientId: string,
  input: SaveContractSubmissionInput,
  ...
  if (!parsed.success) {
    return {
      ok: false,
      error: parsed.error.issues[0]?.message ?? "Dados de contrato inválidos.",
    };
  }
  ...
  if (parsed.data.submissionId) {
    ...
    if (!parsed.data.silent) {
      ...
      revalidatePath(`/pacientes/${cid.data}/contratos`);
    }
...
export async function submitContract(
  ...
  if (!submission) return { ok: false, error: "Contrato não encontrado." };
  ...
  if (submission.template_id) {
    const { data: template } = await ctx.supabase
      ...
      .select("storage_key, mime_type, form_schema")
  ...
  const storagePath = buildContractSubmissionStoragePath({
...
export async function deleteContractSubmission(
  clientId: string,
  submissionId: string,
): Promise<Ok | Err> {
  const ctx = await requireClinicalTenantContext();
  ...
  const { data: sub } = await ctx.supabase
    ...
    .eq("tenant_id", ctx.tenantId)
    .maybeSingle();
  if (!sub) return { ok: false, error: "Contrato não encontrado." };
